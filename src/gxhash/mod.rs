...
pub fn gxhash32(input: &[u8], seed: i64) -> u32 {
    unsafe {
        let p = &gxhash(input, load_i64(seed)) as *const State as *const u32;
        *p
    }
}
...
pub fn gxhash64(input: &[u8], seed: i64) -> u64 {
    unsafe {
        let p = &gxhash(input, load_i64(seed)) as *const State as *const u64;
        *p
    }
}
...
pub fn gxhash128(input: &[u8], seed: i64) -> u128 {
    unsafe {
        let p = &gxhash(input, load_i64(seed)) as *const State as *const u128;
        *p
    }
}
